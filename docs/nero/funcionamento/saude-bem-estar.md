...
- Tipo `bemestar` na central da Fase 3 (`SECOES_BEMESTAR`: perfil, evolução corporal, alimentação, **água**, atividade, sono, check-ins, documentos; pendências ao final). `carregarDadosNero` preenche `DadosNero.bemEstar` (medidas, **água**, atividades, sonos, refeições, check-ins, metas, vínculos, parâmetros, altura/sexo/idade).
...
### 8.1 Ingestão de água no relatório (D-020, 22/09/2026)
- **Água entra; sequência de dias seguidos não.** O relatório é o documento que vai para o médico: água é dado de saúde, comparável a sono e atividade. A sequência é **métrica de adesão ao app**, não achado clínico — não diz nada que o médico precise ler e arriscaria ser lida como desempenho do paciente. Coerente com a D-016, que mantém comemoração e conteúdo clínico em trilhos separados. Há **teste de regressão** garantindo que a sequência não apareça no relatório.
- Seção **água**: texto com a meta em uso e sua origem, tabela por semana (dias registrados, média por dia, total, dias que bateram a meta) e barras da média semanal contra a meta. Sem meta definida, a tabela mostra "—" na coluna de dias batidos e as barras não aparecem — é o caso de quem tem restrição hídrica, para quem o app não calcula volume nenhum.
- **A média divide pelos dias com registro, não por sete.** Um dia sem registro não é um dia sem beber: o app não sabe o que aconteceu e lançar zero puxaria a média para baixo, fazendo o médico ler como ingestão insuficiente algo que é apenas ausência de anotação. Regra em `resumoAguaSemana` (`src/core/regras/bemestar/agua.ts`), com teste próprio.
- O texto da seção repete a ressalva da C-021: é **água bebida**, não água total, por isso não é comparável a EFSA/IOM. Ver `docs/nero/referencias/agua.md`.
- `RegistroAgua` passou a viver no núcleo puro (`regras/bemestar/tipos.ts`), reaproveitado pela camada de dados.
- **Corrigido de quebra:** `src/core/bemestar/__tests__/useAgua.test.tsx` **nunca havia rodado** — estourava na importação (a cadeia de `salvarLembretesAgua` chega ao AsyncStorage) e o Jest a contava como 0 testes, então ela não aparecia no total de 420 citado no handoff de 19/09. Com `lembretesAgua` mockado, os 2 testes rodam e passam.
