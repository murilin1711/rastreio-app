...
select is((select count(*) from public.regras_clinicas where classificacao in ('normal','controle') and intervalo_meses is null and exame_tipo not in ('psa','colonoscopia')), 0::bigint, 'normal/controle têm intervalo, exceto PSA e adenomas (intervalo do laudo)');
select * from finish();
rollback;
