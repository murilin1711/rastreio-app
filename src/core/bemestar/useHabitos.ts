...
import { checkinPendente } from '@core/regras/bemestar/checkin';
...
import { listarRefeicoes } from './refeicoes';
...
export function useHabitos() {
  ...
  const [dados, setDados] = useState<{ atividades: Atividade[]; sonos: Sono[]; corporais: MedidaCorporal[]; metas: Meta[]; refeicoes: Refeicao[]; checkins: Checkin[]; p: ParametrosBemEstar } | null>(null);
  ...
  const recarregar = useCallback(async () => {
    ...
    try {
      ...
      const [atividades, sonos, corporais, metas, refeicoes, checkins, regras] = await Promise.all([listarAtividades(userId, { desde }), listarSono(userId, { desde }), listarCorporais(userId), listarMetas(userId), listarRefeicoes(userId, { desde: desde7 }), listarCheckins(userId), carregarRegrasBemEstar()]);
      setDados({ atividades, sonos, corporais, metas, refeicoes, checkins, p: extrairParametrosBemEstar(regras) });
  ...
  const habitos: Habitos7d | null = useMemo(() => {
    ...
    return habitos7d({ hoje: hojeLocalISO(), p: dados.p, meta: dados.metas.find((m) => m.tipo === 'atividade_min') ?? null, atividades: dados.atividades, sonos: dados.sonos, pesos: dados.corporais.filter((m) => m.tipo === 'peso'), cinturas: dados.corporais.filter((m) => m.tipo === 'cintura'), refeicoes: dados.refeicoes.length, checkinPendente: checkinPendente(dados.checkins, hojeLocalISO()) });
  }, [dados]);
