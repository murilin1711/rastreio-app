...
import { Button, Card, Colors, InternalHeader, Spacing, StatusBadge, Typography } from '@ui/index';
  ...
  return (
    <SafeAreaView style={styles.tela} edges={['top']}>
      <ScrollView contentContainerStyle={styles.conteudo} refreshControl={<RefreshControl refreshing={carregando} onRefresh={recarregar} tintColor={Colors.primary} />}>
        ...
        {pendencias.length === 0 && !carregando ? <Text style={styles.vazio}>Nenhuma pendência. Quando um exame precisar de continuidade, ele aparece aqui.</Text> : null}
          ...
          {pendencias.map((p) => {
            ...
            return (
              <Card key={p.id} style={{ gap: Spacing.sm }}>
                ...
                <Button label="Registrar exame relacionado" variant="outline" onPress={() => router.push({ pathname: '/(app)/rastreando/[programa]/registrar', params: { programa: p.programa, pendencia: p.exameOrigemId } })} />
              </Card>
            );
          })}
        </View>
